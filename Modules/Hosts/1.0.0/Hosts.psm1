...
Function New-HostnameMapping {
    ...
    If (-Not (Get-HostnameMapping -Hostname $Hostname -IPAddress $IPAddress)) {
        If (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
            ...
            Start-Process -FilePath PowerShell -Verb RunAs -ArgumentList "-Command New-HostnameMapping -Hostname $Hostname -IPAddress $IPAddress"
        }
    }
}
...
Function Set-HostnameMapping {
    ...
    If ($Address.Matches) {
        ...
        If (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
            ...
            Start-Process -FilePath PowerShell -Verb RunAs -ArgumentList "-Command Set-HostnameMapping -Hostname $Hostname -NewHostname $NewHostname -NewIPAddress $NewIPAddress"
        }
    }
}
...
Function Remove-HostnameMapping {
    ...
    If (([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        ...
        Start-Process -FilePath PowerShell -Verb RunAs -ArgumentList "-Command Remove-HostnameMapping -Hostname $Hostname"
    }
}